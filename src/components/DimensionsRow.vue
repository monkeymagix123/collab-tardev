<template>
  ...
  <div class="flex w-full items-center p-2 rounded-lg  bg-gradient-to-r from-blue-700/30 via-purple-700/30 to-blue-700/30  rounded-lg mb-3">
    ...
    <div class="flex-1 text-lg font-semibold">
      <span class="font-normal">&times</span><span class="number-display"> {{ styler.writeNumber(game.calculateDimMultiplier(dimension)) }} </span>
    </div>
    <div class="flex-1 text-lg font-semibold">
    ...
    <div class="flex-[1.5] group relative inline-block">
      ...
      >
        Buy 1 for <span> {{ styler.writeNumber(game.calculateDimensionCost(dimension)) }} </span>
      </button>
      ...
      >
        Purchased <span class="number-display">{{ game.dimBought[dimension] }}</span> times
      </div>
    </div>
    <p></p>
  </div>
